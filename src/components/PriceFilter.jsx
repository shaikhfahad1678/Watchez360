...
// Moved outside to avoid garbage collection/recreation on every single render loop
...
export default function PriceFilter({ price, setPrice }) {
  ...
  // Setup Refs for direct DOM access (Bypassing React renders)
  ...
  // Wrapped in a stable useCallback to satisfy React dependency rules safely
  const updateDialRect = useCallback(() => {
  ...
  // Direct DOM mutation function locked to screen refresh rate
  const updateVisuals = useCallback((currentPrice) => {
    ...
    rAFRef.current = requestAnimationFrame(() => {
      ...
      if (handRef.current) {
        handRef.current.style.transform = `rotate(${rotation + 90}deg) translateZ(0)`;
      }
  ...
  const handleInteraction = useCallback((clientX, clientY) => {
    ...
    if (localPriceRef.current !== newPrice) {
      localPriceRef.current = newPrice;
      updateVisuals(newPrice);
    }
  }, [updateVisuals, updateDialRect, min, max]);
  ...
  return (
    <div className="w-full max-w-[320px] pt-5 pb-4 px-5 bg-white rounded-[2.5rem] shadow-lg border border-gray-100 flex flex-col items-center group/card transition-all duration-500">
      ...
      >
        ...
        <div className="absolute w-[84%] h-[84%] rounded-full bg-white shadow-md border border-gray-50 flex items-center justify-center overflow-hidden">
        ...
        <div
          ...
          style={{
            transform: `rotate(${initialRotation + 90}deg) translateZ(0)`,
